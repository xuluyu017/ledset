...
#define UP_KEY_RES      MIC_KEY_UP_RES  //UP按键阻值
#define DOWN_KEY_RES    MIC_KEY_DOWN_RES//DOWN按键阻值
...
/*----------------------------------------------------------------------------*/
/**@brief   另一种按键去抖函数，输出稳定键值
   @param   key：键值
   @return  稳定按键
   @note    u8 other_key_filter(u8 key)
*/
/*----------------------------------------------------------------------------*/
static u8 other_key_filter(u8 key)
{
    u8 return_key = NO_KEY;
    if (key != scan_para.filter_value && scan_para.filter_time) {
        scan_para.filter_cnt = 0;
        scan_para.filter_value = key;
        return_key = NO_KEY;
    }
    ...
    if (scan_para.filter_cnt == 2) {
        return_key = scan_para.filter_value;
    }
    return return_key;
}
...
#if USB_HOST_TYPE_CHECK
        u32 host_type = usb_get_host_type();
        //TYPEC方案,只有Windows主机响应多击按键
        if (host_type != HOST_TYPE_WINDOWS) {
            return 0;//不响应多击
        }
#endif
        return 1;//可响应多击按键
